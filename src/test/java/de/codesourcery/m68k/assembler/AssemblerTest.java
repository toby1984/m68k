...
import de.codesourcery.m68k.assembler.arch.ConditionalInstructionType;
...
{
    private static final Logger LOG = LogManager.getLogger( AssemblerTest.class.getName() );
...
//        assertArrayEquals(compile("move   $12(pc,a0.w*4),d1")    ,0x32,0x3b,0x84,0x12);
    ...
    private byte[] compile(String s)
    {
        ...
        //        LOG.info( "RESULT: "+Memory.hexdump(0,data,0,data.length) );
    ...
    private static void assertArrayEquals(byte[] actual,int...values)
    {
        ...
        if ( fail )
        {
            LOG.info( "EXPECTED: "+Memory.hexdump(0,expected,0,expected.length) );
            LOG.info( "ACTUAL  : "+Memory.hexdump(0,actual,0,actual.length) );
            LOG.info( "EXPECTED: "+Memory.bindump(0,expected,0,expected.length) );
            LOG.info( "ACTUAL  : "+Memory.bindump(0,actual,0,actual.length) );
