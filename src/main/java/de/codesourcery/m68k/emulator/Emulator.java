...
import de.codesourcery.m68k.emulator.ui.ITickListener;
...
{
    private static final Logger LOG = LogManager.getLogger( Emulator.class.getName() );
    ...
    private class EmulatorThread extends Thread
    {
        ...
        private void doReset()
        {
            ...
            LOG.info( "Writing "+kickstartRom.length+" bytes of kickstart ROM to "+ Misc.hex(amiga.getKickRomStartAddress()) );
        ...
        public void run()
        {
            ...
            try
            {
                LOG.info( "Emulator thread start" );
                internalRun();
            ...
            finally
            {
                if ( regularTermination ) {
                    LOG.info(  "Emulator thread finished."  );
                }
                else
                {
                    LOG.info(  "Emulator thread died unexpectedly."  );
                    ...
                    final Thread t = new Thread( () ->
                    {
                        ...
                        synchronized (EMULATOR_LOCK)
                        {
                            if ( emulatorThread == null || !emulatorThread.isAlive() )
                            {
                                LOG.info(  "Restarting emulator thread"  );
                                emulatorThread = new EmulatorThread();
                            ...
                            {
                                LOG.info(  "Emulator thread already running"  );
                            }
                        }
                    } );
                    t.setDaemon( true );
            ...
            while (true)
            {
                ...
                if (cmd != null)
                {
                    ...
                    switch (cmd.type)
                    {
                        ...
                        case CALLBACK:
                            ...
                            {
                                LOG.info( "Emulator callback crashed" );
                                e.printStackTrace();
                            }
                            break;
                        default:
                ...
                if (mode == EmulatorMode.RUNNING)
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        e.printStackTrace();
                        mode = EmulatorMode.STOPPED;
                        LOG.info( "*** emulation stopped because of error ***" );
                        stateCallback.stopped(Emulator.this);
                    }
                    ...
                    if ( cpuBreakpointHit || memory.breakpoints.lastHit != null )
                    {
                        ...
                        if ( cpuBreakpointHit ) {
                            LOG.info( "*** emulation stopped because of CPU breakpoint ***" );
                        }
                        else
                        {
                            LOG.info(  "*** emulation stopped because of memory breakpoint ***"  );
                        }
                        try
                        {
                            stateCallback.stopped( Emulator.this );
    ...
    public void setTickCallback(final ITickListener cb)
    {
        ...
        {
            thread.callback = cb;
            LOG.info( "Emulator callback updated,invoking it" );
    ...
    public void setCallbackInvocationTicks(int callbackInvocationTicks)
    {
        ...
        internalAsyncSendCommand( thread ->
        {
            ...
            LOG.info( "Emulator callback will be invoked every "+callbackInvocationTicks+" ticks." );
            thread.callback.tick(Emulator.this );
        });
    }
