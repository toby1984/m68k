...
// cia A port B (0xbfe101) -- parallel port
    ...
    private int elapsedTodCycles;
    private final int cyclesPerTodTick;

    public CIA8520(CIA8520.Name name, Amiga amiga,IRQController irqController) {
        this.name = name;
...
15 F      CRB           Control register B
        */
        ...
        switch( regNum )
        {
            ...
            case REG_IRQ_CTRL:
                ...
                LOG.info( "IRQ mask is now: "+ Misc.binary8Bit(irqMaskRegister) );
    ...
    public int readRegisterNoSideEffects(int regNum) {
        {
            switch( regNum )
            {
                ...
                case REG_CTRLB:
                    return ctrlB;
                default:
                    LOG.info( "Unhandled register read #" + regNum + " on " + this );
                    return 0;
            }
        }
    }
    ...
    public int readRegister(int regNum)
    {
        if ( DEBUG )
        {
            ...
            LOG.info(  "READ: "+this+" - register "+Integer.toHexString(regNum)+ " ==> "+Integer.toHexString(result)+" (%"+Integer.toBinaryString(result)+")" );
            return result;
        }
                ...
                if ( isSerialOutput() && (++timerAUnderflowCount & 1) != 0 && isTimerAContinous() )
                {
                    ...
                    if ( hasData )
                    {
                        ...
                        if ( cntOut )
                        {
                            ...
                            LOG.info( "BIT OUT: "+serialPin );
                            serialShiftReg <<= 1;
                            shiftRegisterBits--;
                            if (shiftRegisterBits == 0)
                            {
                                LOG.info( "Shift register empty." );
                                triggerInterrupt(ICR_SP);
                            }
                        }
                    }
