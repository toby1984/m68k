...
import de.codesourcery.m68k.emulator.ui.structexplorer.StructTreeModel;
import de.codesourcery.m68k.emulator.ui.structexplorer.StructTreeModelBuilder;
import de.codesourcery.m68k.emulator.ui.structexplorer.StructTreeNode;
...
{
    private static final Logger LOG = LogManager.getLogger( StructExplorer.class.getName() );
    ...
    public void tick(Emulator emulator)
    {
        ...
        if ( newModel != null)
        {
            ...
            runOnEDT( () ->
            {
                ...
                if ( selected != null )
                {
                    LOG.info( "Selected: "+debugString(selected) );
outer:
                    for ( int i = selected.getPathCount()-1 ; i >= 0 ; i--)
                    {
                        ...
                        for ( Iterator<StructTreeNode> iterator = finalModel.iterator(); iterator.hasNext() ; ) {
                            ...
                            if ( node.nodeId == actual.nodeId )
                            {
                                ...
                                LOG.info( "new selection : "+ debugString(newPath)  );
