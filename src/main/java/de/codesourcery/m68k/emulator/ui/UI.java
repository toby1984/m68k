...
import de.codesourcery.m68k.emulator.memory.MemoryBreakpoints;
...
{
    private static final Logger LOG = LogManager.getLogger( UI.class.getName() );
    ...
    private byte[] loadKickstartRom() throws IOException
    {
        ...
        LOG.info( "*** Loading kickstart ROM from "+file.getAbsolutePath() );
    ...
    private void setupEmulator() throws Exception
    {
        ...
        LOG.info( "Setting up new emulator instance for "+Amiga.AMIGA_500 );
        LOG.info( "Using kickstart ROM "+loadConfig().getKickRomLocation() );
    ...
    private void setupLibraryCallResolver(Emulator emulator) throws IOException
    {
        ...
        for (UIConfig.LibraryMapping mapping : config.getLibraryMappings())
        {
            ...
            if ( matches.size() == 1 )
            {
                LOG.info( "Registering "+matches.get(0).getAbsolutePath()+" => "+mapping );
            ...
            LOG.info(  "No matching files for " + mapping.descFileRegex  );
        }
    }
    ...
    private AppWindow registerWindow(AppWindow.WindowKey key)
    {
        ...
        if ( state.isPresent() && ! state.get().isEnabled() ) {
            LOG.info( "Window "+key+" is disabled." );
            return null;
        }
        ...
        if ( state.isPresent() ) {
            LOG.info( "Applying window state "+state.get() );
    ...
    private UIConfig loadConfig()
    {
        if ( uiConfig == null )
        {
            ...
            LOG.info( "Trying to load configuration from "+file.getAbsolutePath() );
            if ( file.exists() )
            {
