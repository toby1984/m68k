...
{
    private static final Logger LOG = LogManager.getLogger( EncodingTableGenerator.class.getName() );
    ...
    public static final class AddressingModeIterator extends EncodingTableGenerator.IValueIterator
    {
        ...
        public AddressingModeIterator(Field field1,Field field2,Set<AddressingMode> modes)
        {
            ...
            LOG.info( "AddressingModeIterator(): eaMode is now "+currentMode() );
            reset();
        }
        ...
        protected void doIncrement()
        {
            if ( currentMode().hasFixedEaRegisterValue() ) {
                modePtr++;
                LOG.info( "doIncrement(1): eaMode is now "+currentMode() );
            ...
            {
                modePtr++;
                LOG.info( "doIncrement(2): eaMode is now "+currentMode() );
                resetEaRegisterValue();
            }
            else
            {
                LOG.info( "doIncrement(3): eaRegister is now " + Misc.binary3Bit(eaRegisterValue) );
            }
        }
        ...
        private void resetEaRegisterValue() {
            ...
            LOG.info( "reset(): eaRegister is now "+Misc.binary3Bit(eaRegisterValue) );
        }
        ...
        {
            modePtr = 0;
            LOG.info( "reset(): eaMode is now "+currentMode() );
            resetEaRegisterValue();
        }
    }
    ...
    public void addMappings(InstructionEncoding encoding,Instruction instruction,CPUType cpuType,Map<Integer, EncodingEntry> result)
    {
        ...
        {
            LOG.info( "Getting iterator for "+fields );
        ...
        while(true)
        {
            ...
            LOG.info( position+": "+Misc.binary16Bit(value)+" => "+instruction.name() );
            position++;
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        for ( var entry : allEncodings.entrySet() )
        {
            ...
            for (InstructionEncoding enc : entry.getValue() )
            {
                ...
                {
                    LOG.info( "Processing encoding: " + getName(enc) );
        ...
        mappings.values().stream().map( x -> getName(x.encoding) )
            ...
            .forEach(name ->
            {
                ...
                LOG.info( "private final InstructionImpl "+name+" = "+impl+";\n" );
            });
...
//            .sorted()
//            .forEach(pair ->
//                LOG.info( "insnImplementation.put(\""+pair.a+"\",null );")
//            );
    }
    ...
    private static void sanityCheck(Instruction insn,int opcode) {
        ...
        Video video=new Video(amiga,blitter,dmaCtrl);
        ...
        if ( ! matcher.matches() ) {
            LOG.info( "No regex match for '"+ disasm[0]+"'" );
        ...
        if ( ! actual.toUpperCase().equals(expected) )
        {
            ...
            {
                LOG.info( "DISASSEMBLY: " + Misc.binary16Bit(opcode) + " => (" + actual + ") " + disasm[0] );
